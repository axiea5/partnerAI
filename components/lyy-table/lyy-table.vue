		...
		<uni-table stripe :loading="loading">
				...
				<uni-th v-for="(item,index) in headers" :key="index" align="center" :width="item.width"
					...
					<view style="display: flex;flex-direction: row;justify-content: center;">
						...
						<view class="header-icon"
							:style="{lineHeight:'20rpx',display:'flex',flexDirection:'column',marginLeft:'10rpx'}"
							v-if="item.sort">
							<text class="iconfont icon-arrow-up"
								:style="{fontSize:'20rpx',height:'20rpx',color:lastSortItem===item.format.keys[0]&&sortWay=='asc'?'#333':'#bcbcbc'}" />
							<text class="iconfont icon-arrow-down"
								:style="{fontSize:'20rpx',height:'20rpx',color:lastSortItem===item.format.keys[0]&&sortWay=='desc'?'#333':'#bcbcbc'}" />
						</view>
					</view>
				</uni-th>
			</uni-tr>
			<tbody id="lyy-tbody" style="display:block;overflow-y:auto">
				...
				<uni-tr v-else v-for="(content,sindex) in sortContents" :key="sindex"
					style="width:100%;display:table;table-layout: fixed;" @click.native="rowClick(content)">
					<uni-td v-for="(header,hindex) in headers" :key="hindex"
						:style="{display:header.hidden?'none':'table-cell',textAlign:'center',width:header.width+'px'}">
						<lyy-progress
							v-if="typeof header.format!=='undefined'&& header.format.type==='progress'&&!isNaN(parseFloat(content[header.key]))"
							:percent="content[header.key].toFixed(2)" show-info round></lyy-progress>

						<view v-else-if="typeof header.format!=='undefined'&& header.format.type==='html'"
							v-html="content[header.key]"></view>
						<text
							v-else-if="typeof header.format!=='undefined'&& header.format.type==='index'">00{{sindex+1}}</text>
...
<script>
	 ...
	 *  @value format 格式化
	 *   @value type = [string|html|conpute|progress|index]
	 *    @value string 字符串格式化
	 *    @value html html格式化
	 *    @value compute 计算格式化
